...
UPLOAD_RESP=$(curl -s -X POST "https://api.minimaxi.com/v1/files/upload" \
...
if [ -n "$SAMPLE_AUDIO" ]; then
  # 上传示例音频
  SAMPLE_RESP=$(curl -s -X POST "https://api.minimaxi.com/v1/files/upload" \
    -H "Authorization: Bearer ${MINIMAX_API_KEY:-$(security find-generic-password -s minimax -w 2>/dev/null)}" \
    -F "purpose=voice_clone" \
    -F "file=@$SAMPLE_AUDIO")
...
CLONE_RESP=$(curl -s -X POST "https://api.minimaxi.com/v1/voice_clone" \
  -H "Authorization: Bearer ${MINIMAX_API_KEY:-$(security find-generic-password -s minimax -w 2>/dev/null)}" \
